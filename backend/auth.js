...
export async function sessionUser(req) {
  if (databaseMode() !== 'postgres') return null;
  const token = cookies(req)[COOKIE];
  if (!token) return null;
  try {
    return await getSessionUser(hashToken(token));
  } catch {
    return null;
  }
}
...
export async function requireUser(req, res, next) {
  if (databaseMode() !== 'postgres') {
    return res.status(503).json({ erro: 'O banco do produto ainda não foi configurado.' });
  }
...
export function registerAuthRoutes(app, { sanitizeProfile }) {
  app.get('/api/auth/me', async (req, res) => {
    if (databaseMode() !== 'postgres') return res.status(503).json({ erro: 'Banco ainda não configurado.' });
    const user = await sessionUser(req);
  ...
  app.post('/api/auth/logout', async (req, res) => {
    const token = cookies(req)[COOKIE];
    if (token && databaseMode() === 'postgres') {
      try { await deleteSession(hashToken(token)); } catch {}
    }
  ...
  app.get('/api/profile', requireUser, async (req, res) => {
    try {
      const profile = sanitizeProfile(await getUserProfile(req.user.id));
      res.json({ profile });
    } catch (e) {
  ...
  app.put('/api/profile', requireUser, async (req, res) => {
    try {
      ...
      await saveUserProfile(req.user.id, profile);
      res.json({ profile });
    } catch (e) {
      console.error('Erro ao salvar perfil:', e.message);
      res.status(500).json({ erro: 'Não foi possível salvar o perfil.' });
    }
  });
}
