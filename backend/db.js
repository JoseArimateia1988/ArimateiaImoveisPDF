...
function requirePostgres() {
  const p = getPool();
  if (!p) { const e = new Error('Contas exigem DATABASE_URL configurada.'); e.code = 'AUTH_REQUIRES_POSTGRES'; throw e; }
  return p;
}
...
export function databaseMode() { if (DATABASE_URL) return 'postgres'; if (SUPA_URL && SUPA_KEY) return 'supabase'; return 'none'; }
...
export async function createUser({ id, email, passwordHash, profile = {} }) {
  const p=requirePostgres();await ensureSchema();const client=await p.connect();
...
export async function findUserByEmail(email){const p=requirePostgres();await ensureSchema();const{rows}=await p.query(`SELECT u.id,u.email,u.password_hash,COALESCE(p.data,'{}'::jsonb) AS profile FROM users u LEFT JOIN user_profiles p ON p.user_id=u.id WHERE LOWER(u.email)=LOWER($1) LIMIT 1`,[email]);return rows[0]||null;}
export async function createSession({tokenHash,userId,expiresAt}){const p=requirePostgres();await ensureSchema();await p.query(`DELETE FROM sessions WHERE expires_at<=NOW()`);await p.query(`INSERT INTO sessions (token_hash,user_id,expires_at) VALUES ($1,$2,$3)`,[tokenHash,userId,expiresAt]);}
export async function getSessionUser(tokenHash){const p=requirePostgres();await ensureSchema();const{rows}=await p.query(`SELECT u.id,u.email,COALESCE(p.data,'{}'::jsonb) AS profile FROM sessions s JOIN users u ON u.id=s.user_id LEFT JOIN user_profiles p ON p.user_id=u.id WHERE s.token_hash=$1 AND s.expires_at>NOW() LIMIT 1`,[tokenHash]);return rows[0]||null;}
export async function deleteSession(tokenHash){const p=requirePostgres();await ensureSchema();await p.query(`DELETE FROM sessions WHERE token_hash=$1`,[tokenHash]);}
export async function getUserProfile(userId){const p=requirePostgres();await ensureSchema();const{rows}=await p.query(`SELECT data FROM user_profiles WHERE user_id=$1 LIMIT 1`,[userId]);return rows[0]?.data||{};}
export async function saveUserProfile(userId,profile){const p=requirePostgres();await ensureSchema();await p.query(`INSERT INTO user_profiles (user_id,data,updated_at) VALUES ($1,$2::jsonb,NOW()) ON CONFLICT (user_id) DO UPDATE SET data=EXCLUDED.data,updated_at=NOW()`,[userId,JSON.stringify(profile||{})]);return profile||{};}
...
export async function saveVotes(id,votos){
  ...
  if(p){
    await ensureSchema();
    const r=await p.query(`UPDATE presentations SET votes=$2::jsonb,updated_at=NOW() WHERE id=$1 AND votes IS NULL`,[id,JSON.stringify(votos)]);
    if(!r.rowCount){const existing=await p.query(`SELECT votes FROM presentations WHERE id=$1 LIMIT 1`,[id]);if(!existing.rowCount)throw new Error('Apresentação não encontrada');const e=new Error('Avaliação já enviada');e.code='VOTES_ALREADY_SENT';throw e;}
    return;
  }
  const current=await getPresentation(id);if(!current)throw new Error('Apresentação não encontrada');if(current.votos){const e=new Error('Avaliação já enviada');e.code='VOTES_ALREADY_SENT';throw e;}
  const r=await supa(`apresentacoes?id=eq.${encodeURIComponent(id)}`,{method:'PATCH',body:JSON.stringify({votos})});if(!r.ok)throw new Error(await r.text());
}
