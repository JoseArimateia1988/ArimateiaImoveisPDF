...
function perfil(raw = {}) {
  return {
    ...
    corPrincipal: /^#[0-9a-f]{6}$/i.test(raw.corPrincipal || '') ? raw.corPrincipal : '#243b2a',
    corSecundaria: /^#[0-9a-f]{6}$/i.test(raw.corSecundaria || '') ? raw.corSecundaria : '#7a4f2d',
    usarCores: !!raw.usarCores,
  };
}

export function errorPage(msg) {
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Apresentação</title><style>body{margin:0;background:#d8d4cc;font-family:system-ui;display:grid;place-items:center;min-height:100vh}.box{background:#f7f4ee;padding:28px;border-radius:12px;max-width:520px}.box h2{color:#243b2a}</style></head><body><div class="box"><h2>Apresentação</h2><p>${esc(msg)}</p></div></body></html>`;
}
...
export function clientPage(entrada) {
  ...
  const primary = p.usarCores ? p.corPrincipal : '#243b2a';
  const accent = p.usarCores ? p.corSecundaria : '#7a4f2d';
  const imoveisJson = JSON.stringify(entrada.imoveis || []).replace(/<\/script>/gi, '<\\/script>');
  ...
  const contato = [p.creci, p.whatsapp, p.instagram, p.email].filter(Boolean).join(' · ');
...
async function sendVotes(){const b=document.getElementById('enviar');b.disabled=true;b.textContent='Enviando…';try{const r=await fetch('/api/votar/'+ID,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({votos:V})});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.erro||'Não consegui enviar.');document.getElementById('lista').innerHTML='<div class="done"><h2>✓ Avaliação enviada</h2><p>As suas escolhas já estão disponíveis para o corretor.</p></div>';document.getElementById('barra').remove()}catch(err){b.disabled=false;b.textContent='Enviar avaliação';alert(err.message||'Não consegui enviar. Tente novamente.')}}render();
</script></body></html>`;
}
...
export function resultPage(entrada) {
  ...
  const ok = (entrada.imoveis || []).map((i, idx) => i?.ok ? { ...i.dados, idx } : null).filter(Boolean);
  const sections = [
    ['Gostou', ok.filter(d => votes[d.idx] === 'like'), '#243b2a'],
    ['Não é pra mim', ok.filter(d => votes[d.idx] === 'dislike'), '#953c3c'],
    ['Sem avaliação', ok.filter(d => !votes[d.idx]), '#888'],
  ];
  const body = sections.map(([title, list, color]) => `<section><h2 style="color:${color}">${title} (${list.length})</h2>${list.length ? list.map(d => {
    const photo = (d.fotos || []).find(Boolean);
    const price = d.preco_venda || d.preco_aluguel || (d.tipologias || []).map(t => t.preco_venda || t.preco_aluguel).find(Boolean) || '';
    return `<div class="item">${photo ? `<img src="${esc(photo)}">` : ''}<div><small>${esc(d.codigo || '')}</small><strong>${esc(d.titulo || 'Imóvel')}</strong><span>${esc(price)}</span>${d.url_origem ? `<a href="${esc(d.url_origem)}" target="_blank">Ver anúncio ↗</a>` : ''}</div></div>`;
  }).join('') : '<p class="none">Nenhum</p>'}</section>`).join('');

  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Resultado</title><style>body{margin:0;background:#d8d4cc;font-family:system-ui;color:#222}.wrap{width:min(720px,calc(100% - 24px));margin:22px auto}.head{background:#243b2a;color:#ede8df;padding:20px;border-bottom:4px solid #7a4f2d}.head h1{margin:0}.head p{opacity:.7}.body{background:#ede8df;padding:20px}section h2{font-size:12px;text-transform:uppercase;border-bottom:2px solid currentColor;padding-bottom:6px}.item{display:flex;gap:12px;background:#fff;padding:10px;margin:7px 0;border-radius:6px}.item img{width:76px;height:76px;object-fit:cover}.item div{display:flex;flex-direction:column;gap:3px}.item small{color:#7a4f2d;font-weight:800}.item strong{color:#243b2a}.item a{font-size:12px;color:#243b2a}.none{color:#888}</style></head><body><main class="wrap"><header class="head"><h1>${esc(entrada.cliente ? `Seleção de ${entrada.cliente}` : 'Resultado da seleção')}</h1><p>${esc(p.nome || p.marca || 'Corretor')} · ${esc(p.marca || '')}</p></header><div class="body"><a href="/ver/${esc(entrada.id)}" target="_blank">Abrir como o cliente vê ↗</a>${body}</div></main></body></html>`;
}
