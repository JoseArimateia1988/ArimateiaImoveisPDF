...
const PERFIL_PADRAO = {
  marca: '', nome: '', creci: '', whatsapp: '', instagram: '', email: '',
  foto: '', logo: '', corPrincipal: '#243b2a', corSecundaria: '#7a4f2d', usarCores: false,
};
...
function safeProfile(raw = {}) {
  ...
  return {
    ...
    instagram: text(p.instagram), email: text(p.email),
    foto: /^https?:\/\//i.test(p.foto || '') ? text(p.foto, 1200) : '',
    logo: /^https?:\/\//i.test(p.logo || '') ? text(p.logo, 1200) : '',
    corPrincipal: color(p.corPrincipal, '#243b2a'), corSecundaria: color(p.corSecundaria, '#7a4f2d'),
    usarCores: !!p.usarCores,
  };
}
...
app.post('/api/extrair', requireUser, async (req, res) => {
  ...
  res.json({
    imoveis: resultados.map((r, i) => r.status === 'fulfilled'
...
app.post('/api/salvar', requireUser, async (req, res) => {
  ...
  try {
    await savePresentation({
      ...
      perfil: safeProfile(req.user.profile || {}),
      userId: req.user.id,
    });
    res.json({ id });
  } catch (e) {
...
function resumo(imoveis) {
  ...
  return {
    ...
    preco: d.preco_venda || d.preco_aluguel || (d.tipologias || []).map(t => t.preco_venda || t.preco_aluguel).find(Boolean) || null,
  };
}

app.get('/api/apresentacoes', requireUser, async (req, res) => {
  try {
    const rows = await listPresentations({ userId: req.user.id, limit: 150 });
    res.json(rows.map(r => ({
      id: r.id,
      cliente: r.cliente || null,
      modelo: r.modelo || 'editorial',
      criado_em: r.criado_em || null,
      resumo: resumo(r.imoveis || []),
    })));
  } catch (e) {
...
app.get('/ver/:id', async (req, res) => {
  try {
    ...
    if (!entrada) return res.status(404).send(errorPage('Apresentação não encontrada.'));
    res.send(clientPage(entrada));
  } catch (e) {
    console.error('Erro ao abrir apresentação:', e.message);
    res.status(500).send(errorPage('Erro ao carregar apresentação.'));
  }
});
...
app.post('/api/votar/:id', async (req, res) => {
  ...
  if (keys.length > 100 || keys.some(k => !['like', 'dislike'].includes(votos[k]))) return res.status(400).json({ erro: 'Avaliação inválida.' });
  try {
...
app.get('/resultado/:id', async (req, res) => {
  try {
    const entrada = await getPresentation(req.params.id);
    if (!entrada) return res.status(404).send(errorPage('Resultado não encontrado.'));
    res.send(resultPage(entrada));
  } catch {
    res.status(500).send(errorPage('Erro ao carregar resultado.'));
  }
});
...
function clientPage(entrada) {
  const p = safeProfile(entrada.perfil);
  const model = MODELOS.has(entrada.modelo) ? entrada.modelo : 'editorial';
