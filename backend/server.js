...
function safeProfile(raw = {}) {
  ...
  return {
    ...
    corPrincipal: color(p.corPrincipal, '#243b2a'),
    corSecundaria: color(p.corSecundaria, '#7a4f2d'),
...
app.post('/api/extrair', requireUser, async (req, res) => {
  ...
  const resultados = await Promise.allSettled(urls.map(async (url, i) => {
    ...
    if (sources[i] === 'orulo') {
      return { dados: { ...(await fetchOruloImovel(url)), url_origem: url }, usage: null };
    }
...
app.get('/api/usage', requireUser, async (req, res) => {
  try {
    res.json(await usageSummary(req.user.id, { days: Number(req.query.days || 30) }));
  } catch (e) {
    console.error('Erro ao ler uso:', e.message);
    res.status(500).json({ erro: 'Não foi possível carregar o uso.' });
  }
});
...
app.post('/api/salvar', requireUser, async (req, res) => {
  ...
  // 16 caracteres deixam o link curto para WhatsApp sem ficar fácil de adivinhar.
...
// Link compartilhado com o cliente. Continua público por definição.
...
app.post('/api/votar/:id', async (req, res) => {
  ...
  try {
    ...
    res.status(e?.code === 'VOTES_ALREADY_SENT' ? 409 : 500).json({
      erro: e?.code === 'VOTES_ALREADY_SENT' ? 'Esta avaliação já foi enviada.' : 'Erro ao salvar avaliação.',
    });
  }
});

// O resultado é do corretor. Exige login e confirma que a apresentação pertence à conta.
app.get('/resultado/:id', requireUser, async (req, res) => {
  try {
    ...
    console.error('Erro ao carregar resultado:', e.message);
